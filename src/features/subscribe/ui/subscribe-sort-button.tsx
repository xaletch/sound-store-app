...
export const SubscribeSortButton = ({ name, select }: SubscribeSortButtonProps) => {
  ...
  return (
    <button className={`flex-1 px-6 h-full rounded-full text-xs duration-300 cursor-pointer ${select === name ? 'bg-[#1d1f22] text-white' : 'text-[#1d1f22]'}`} onClick={()=>handleSelect(name)}>{name}</button>
  )
}
